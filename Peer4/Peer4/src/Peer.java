...
public class Peer {
	...
	public static void main(String[] args) throws Exception {
		...
		File file = new File(".\\Files\\" + Peer.filename);

		if ((fileSize = file.length()) != 0) {
			long tradeNum = fileSize / BUFFER_SIZE;
			remain = (int) (fileSize % BUFFER_SIZE);
			remain = (remain == 0) ? 0 : 1;
			tradeNum += remain;
			remain = (int) (tradeNum % 32);
			bitmapLength = (int) (tradeNum / 32) + ((remain != 0) ? 1 : 0);
		...
		if (Peer.fileSize == 0) {
			...
			while (Peer.isNotFinished()) {
				Thread.sleep(1000);
				if (fileSize != 0 && totalReadBytes != 0) {
